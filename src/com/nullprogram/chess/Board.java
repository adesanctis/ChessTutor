...
import com.nullprogram.chess.uciplayer.ChessBoard;
...
public abstract class Board implements Serializable {
    ...
    public final void move(final Move move) {
        if (move==null || move.getOrigin()==null || move.getDest() == null) {
...
//                    System.out.println(p.getPosition()+" "+p.getClass().getName()+" "+p.getMoves(check).size());
    ...
    public final MoveList allCaptureMoves(final Piece.Side side, final boolean check) {
        MoveList list = new MoveList(this, false);
        for (int y = 0; y < boardHeight; y++) {
            for (int x = 0; x < boardWidth; x++) {
                Piece p = board[x][y];
                if (p != null && p.getSide() == side) {
                    list.addAll(p.getCaptureMoves(check));
                }
            }
        }
        return list;
    ...
    public final String moveHistory() {
        StringBuilder moveString = new StringBuilder();
        for (Move move : moves) {
            moveString.append(move).append(" ");
        }
        ...
        for (int r = 7; r > -1; r--) {
            for (int c = 0; c < 8; c++) {
                ...
                if (board[c][r]==null) {
                    ...
                    val = board[c][r].getFENDescriptor();
                }
