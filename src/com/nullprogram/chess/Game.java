...
public class Game implements Runnable {
    ...
    private UciAnalyzer analyzer = new UciAnalyzer(this);
    ...
    public final void begin() {
        ...
        callGameListeners(GameEvent.TURN, "Game Started");
    ...
    public final void run() {
        while (!done) {
            ...
            opponent.opponentTurn(board, move, Piece.opposite(turn));
            board.move(move);
            setProgress(0);
            if (done) {
            ...
            Piece.Side opp = Piece.opposite(getTurn());
            if (board.checkmate(opp)) {
                ...
                callGameListeners(GameEvent.GAME_END, "Game ended!");
                return;
            } else if (board.stalemate(opp)) {
                done = true;
                setStatus("Stalemate!");
                winner = null;
                callGameListeners(GameEvent.GAME_END, "Stalemate!");
                return;
            }
            callGameListeners(GameEvent.TURN, "Change Turn");
        }
    }
     ...
     */
    private void callGameListeners(final int type, String message) {
        for (GameListener listener : listeners) {
            listener.gameEvent(new GameEvent(this, type, message));
        }
    }
    ...
    public final void setStatus(final String message) {
        ...
        status = message;
        callGameListeners(GameEvent.STATUS, message);
    }
    ...
    public final void setProgress(final float value) {
        ...
        callGameListeners(GameEvent.STATUS, null);
    }
