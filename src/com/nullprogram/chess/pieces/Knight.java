...
public class Knight extends Piece {
    ...
    public final MoveList getCaptureMoves(final boolean check) {
        MoveList list = new MoveList(getBoard(), check);
        list = getCaptureMoves(this, list);
        return list;
    }
    ...
    public static MoveList getCaptureMoves(final Piece p,
                                    final MoveList list) {
        Position pos = p.getPosition();
        list.addCaptureOnly(new Move(pos, new Position(pos,  NEAR,  FAR)));
        list.addCaptureOnly(new Move(pos, new Position(pos,  FAR,  NEAR)));
        list.addCaptureOnly(new Move(pos, new Position(pos, -FAR,  NEAR)));
        list.addCaptureOnly(new Move(pos, new Position(pos, -FAR, -NEAR)));
        list.addCaptureOnly(new Move(pos, new Position(pos,  FAR, -NEAR)));
        list.addCaptureOnly(new Move(pos, new Position(pos,  NEAR, -FAR)));
    ...
    @Override
    public String getFENDescriptor() {
        return this.getSide().equals(Side.BLACK)? "n" : "N";
    }
